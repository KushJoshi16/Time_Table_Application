...
def neighbour(chromosome):
    ...
    for k in range(len(chromosome[0])):     # Search for all classes violating hard constraints
        for j in range(len(chromosome[2][chromosome[0][k]['Assigned_class']])):
            if chromosome[2][chromosome[0][k]['Assigned_class']][j] >= 2:
                candidates.append(k)
    ...
    for j in range(chromosome[0][i]['Assigned_time'], chromosome[0][i]['Assigned_time'] + int(chromosome[0][i]['Duration'])):
        ...
        chromosome[2][chromosome[0][i]['Assigned_class']][j] -= 1
        for group in chromosome[0][i]['Group']:
            chromosome[3][group][j] -= 1
    chromosome[4][chromosome[0][i]['Subject']][chromosome[0][i]['Type']].remove((chromosome[0][i]['Assigned_time'], chromosome[0][i]['Group']))
    ...
    if found:
        ...
        chromosome[0][i]['Assigned_class'] = novo[1]
    ...
    for j in range(chromosome[0][i]['Assigned_time'], chromosome[0][i]['Assigned_time'] + int(chromosome[0][i]['Duration'])):
        ...
        for group in chromosome[0][i]['Group']:
            chromosome[3][group][j] += 1
    chromosome[4][chromosome[0][i]['Subject']][chromosome[0][i]['Type']].append((chromosome[0][i]['Assigned_time'], chromosome[0][i]['Group']))
...
def neighbour2(chromosome):
    ...
    for j in range(first['Assigned_time'], first['Assigned_time'] + int(first['Duration'])):
        ...
        chromosome[2][first['Assigned_class']][j] -= 1
        for group in first['Group']:
            chromosome[3][group][j] -= 1
    chromosome[4][first['Subject']][first['Type']].remove((first['Assigned_time'], first['Group']))

    for j in range(second['Assigned_time'], second['Assigned_time'] + int(second['Duration'])):
        chromosome[1][second['Professor']][j] -= 1
        chromosome[2][second['Assigned_class']][j] -= 1
        for group in second['Group']:
            chromosome[3][group][j] -= 1
    chromosome[4][second['Subject']][second['Type']].remove((second['Assigned_time'], second['Group']))
    ...
    tmp_ucionica = first['Assigned_class']
    first['Assigned_class'] = second['Assigned_class']
    second['Assigned_class'] = tmp_ucionica
    ...
    for j in range(first['Assigned_time'], first['Assigned_time'] + int(first['Duration'])):
        ...
        for group in first['Group']:
            chromosome[3][group][j] += 1
    chromosome[4][first['Subject']][first['Type']].append((first['Assigned_time'], first['Group']))
    ...
    for j in range(second['Assigned_time'], second['Assigned_time'] + int(second['Duration'])):
        ...
        for group in second['Group']:
            chromosome[3][group][j] += 1
    chromosome[4][second['Subject']][second['Type']].append((second['Assigned_time'], second['Group']))
